...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        start = time.time()
        flow_pr = model(image1, image2, 192)
        end = time.time()
...
def validate_sceneflow(model):
    ...
    for i_batch, (_, *data_blob) in enumerate(tqdm(val_loader)):
        ...
        disp_pred = model(image1, image2, 192)
...
def validate_middlebury(model, iters=32, split='MiddEval3', resolution='F', mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        flow_pr = model(image1, image2, args.max_disp)
        flow_pr = padder.unpad(flow_pr).cpu().squeeze(0)
    ...
    # f = open('test_middle_train_scale.txt', 'a')
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(BANet(args))
